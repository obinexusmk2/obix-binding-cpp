...
`@obinexusltd/obix-binding-cpp` is a TypeScript binding that connects the **libpolycall FFI /
polyglot ABI bridge** to a C++ runtime, for legacy system integration and embedded targets.
...
5. Heap Tracking and the Library Registry
...
| `heapTracker` | `HeapTrackerAPI` | C++ heap allocation counters |
